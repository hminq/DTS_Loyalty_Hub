...
export function EventDefinitionsFilters({
  filters,
  onApply,
  onClear,
  options = {},
  ...
  const activeFilters = getActiveFilterChips(filters, { options, t })
  ...
  const form = (
    ...
    >
      <div className={presentation === 'popover' ? 'grid gap-3' : 'grid items-end gap-3 md:grid-cols-2 xl:grid-cols-3'}>
        <FilterField label={t('eventDefinitions.filters.searchLabel')}>
          <Input
            value={draft.keyword}
            onChange={(event) => update('keyword', event.target.value)}
            placeholder={t('eventDefinitions.filters.searchPlaceholder')}
            maxLength={100}
          />
        </FilterField>

        <FilterField label={t('eventDefinitions.filters.statusLabel')}>
          <Combobox
            value={draft.status}
            onValueChange={(value) => update('status', value)}
            options={options?.statusOptions ?? []}
            placeholder={t('eventDefinitions.filters.allStatuses')}
            emptyOptionLabel={t('eventDefinitions.filters.allStatuses')}
            isLoading={isLoadingOptions}
            error={optionsError}
            ariaLabel={t('eventDefinitions.filters.statusLabel')}
            shouldFilter={false}
          />
        </FilterField>
...
function getActiveFilterChips(filters, { options, t }) {
  ...
  if (filters.keyword) {
    chips.push({
      key: 'keyword',
      label: t('eventDefinitions.filters.searchLabel'),
  ...
  if (filters.status) {
    chips.push({
      key: 'status',
      label: t('eventDefinitions.filters.statusLabel'),
      value: getOptionLabel(options?.statusOptions, filters.status),
    })
  }
