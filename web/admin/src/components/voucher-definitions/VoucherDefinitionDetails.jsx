...
function VoucherDefinitionDetails({ voucher, language, canImportVoucherCodes, onImportVoucherCodes, t }) {
  ...
  const isAutoGeneratedPrivate =
    voucher.publishType === 'PRIVATE' && voucher.generationType === 'AUTO_GENERATED'
  ...
  const showProvisioningCard = isImportedPrivate || isAutoGeneratedPrivate
  ...
  return (
    <div className="mt-5 flex flex-col gap-5">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        {/* Right Column */}
        <div className="flex flex-col gap-5 lg:contents">
          {/* Reward and Validity Rules Card */}
          <Card className="rounded-xl border-border/80 shadow-none lg:col-start-2 lg:row-start-1">
            <CardHeader className="p-4 pb-3">
              <CardTitle className="text-sm">{t('voucherDefinitions.detail.rulesTitle')}</CardTitle>
            </CardHeader>
            <CardContent className="flex flex-col gap-4 px-4 pb-4 text-[13px]">
              ...
              <div className="grid gap-4 border-t border-border/50 pt-4 sm:grid-cols-2">
                ...
                />
                <div className="grid gap-4 border-t border-border/50 pt-4 sm:col-span-2 sm:grid-cols-2">
          ...
          {showProvisioningCard ? (
            <div className="lg:col-start-2 lg:row-start-3">
              <VoucherPoolProvisioningCard
                type={isImportedPrivate ? 'import' : 'generation'}
                provisioning={provisioning}
                language={language}
                showImportAction={showImportAction}
                onImportVoucherCodes={onImportVoucherCodes}
                t={t}
              />
            </div>
          ) : null}
...
function VoucherPoolProvisioningCard({
  type,
  provisioning,
  language,
  showImportAction,
  onImportVoucherCodes,
  t,
}) {
  const isImport = type === 'import'
  const statusTranslationScope = isImport ? 'import' : 'generation'
  const progressPercentage = provisioning?.expectedCount > 0
    ? Math.min(100, (provisioning.processedCount / provisioning.expectedCount) * 100)
    : 0
  ...
  return (
    <Card
      className={
        isImport
          ? 'rounded-xl border-[#217346]/25 bg-[#217346]/[0.025] shadow-none'
          : 'rounded-xl border-primary/20 bg-primary/[0.025] shadow-none'
      }
    >
      <CardHeader className="p-4 pb-3">
        <div className="flex items-start gap-3">
          <div
            className={
              isImport
                ? 'flex size-9 shrink-0 items-center justify-center rounded-lg bg-[#217346]/10 text-[#217346]'
          ...
          >
            ...
            <GearSixIcon size={20} weight="fill" aria-hidden="true" />
            )}
          </div>
          <div>
            <CardTitle className="text-sm">
              {t(
                isImport
                  ? 'voucherDefinitions.detail.voucherCodeImportTitle'
                  : 'voucherDefinitions.detail.voucherCodeGenerationTitle',
              )}
            </CardTitle>
            <CardDescription className="mt-1 text-xs leading-5">
              {t(
                isImport
                  ? 'voucherDefinitions.detail.voucherCodeImportDescription'
                  : 'voucherDefinitions.detail.voucherCodeGenerationDescription',
              )}
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="flex flex-col gap-3 px-4 pb-4 text-[13px]">
        {provisioning ? (
          <div className="rounded-lg border border-border/70 bg-background/70 px-3 py-2.5">
            <div className="flex items-center justify-between gap-3">
              <Badge variant={getProvisioningBadgeVariant(provisioning.status)}>
                {t(`voucherDefinitions.${statusTranslationScope}.status.${provisioning.status}`, {
            ...
            {provisioning.status === 'FAILED' && provisioning.errorCode ? (
              <p className="mt-2 text-xs font-medium text-destructive">
                {t(`voucherDefinitions.${statusTranslationScope}.errors.${provisioning.errorCode}`, {
        ...
        {showImportAction ? (
          ...
          >
            <MicrosoftExcelLogoIcon data-icon="inline-start" weight="fill" aria-hidden="true" />
...
function getProvisioningBadgeVariant(status) {
