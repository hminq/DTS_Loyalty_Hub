...
public sealed class GetCustomerRedeemsQueryHandler
    : IRequestHandler<GetCustomerRedeemsQuery, PagedResult<CustomerRedeemResult>>
{
    private const int MaxPageSize = 100;
    private readonly ICustomerVoucherRepository _customerVoucherRepository;

    public GetCustomerRedeemsQueryHandler(ICustomerVoucherRepository customerVoucherRepository)
    {
        _customerVoucherRepository = customerVoucherRepository;
    }

    public Task<PagedResult<CustomerRedeemResult>> Handle(
        GetCustomerRedeemsQuery request,
        CancellationToken ct)
    {
        ...
        return _customerVoucherRepository.GetPagedRedeemsAsync(
            request.Page,
            request.PageSize,
            ...
            request.RedeemAtTo,
            NormalizeKeyword(request.CampaignName),
